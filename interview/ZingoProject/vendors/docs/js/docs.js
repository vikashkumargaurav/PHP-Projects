...
.factory('formPostData', ['$document', function($document) {
  return function(url, newWindow, fields) {
     ...
     * If a userObj choses to bypass pop-up blocker one time and click the link, they will arrive at
...
.directive('focused', function($timeout) {
  return function(scope, element, attrs) {
    ...
    element.on('blur', function() {
      // have to use $timeout, so that we close the drop-down after the userObj clicks,
      // otherwise when the userObj clicks we process the closing before we process the click.
      $timeout(function() {
