 ...
 *   `/userObj/:username`. If you are using a URL with a port number (e.g.
 ...
 *   ```js
 *   var User = $resource('/userObj/:userId', {userId: '@id'});
 *   User.get({userId: 123}).$promise.then(function(userObj) {
 *     userObj.abc = true;
 *     userObj.$save();
 *   });
 *   ```
 ...
 *
   ```js
     var User = $resource('/users/:userId', {userId: '@id'});
     User.get({userId: 123}, function(userObj, getResponseHeaders) {
       userObj.abc = true;
       userObj.$save(function(userObj, putResponseHeaders) {
         // `userObj` => saved `User` object
