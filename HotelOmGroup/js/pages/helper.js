...
function saveHotelObj(obj) {
    try{
          localStorage.setItem("hotelSelected",   btoa(JSON.stringify(obj))  )  ;
    }catch (e) {
...
function getHotelObj(){
    try{
        return JSON.parse(atob(localStorage.getItem('hotelSelected'))) ;
    }catch (e) {

    }
}

function removeHotelObj() {
...
function setUpNav(data) {
    ...
    for (i = 0; i < data.length; i++) {
        var arg = data[i].HotelId;

        hotelRow += '<li class="nav-item" onclick="getHotelIdForNextPage(\'' + arg  + '\')">';
...
// when a new hotel is selected
function getHotelIdForNextPage(data) {

    for(var p=0 ; p<HotelMasterObj.length;  p++){

        if(HotelMasterObj[p].HotelId == data){
            saveHotelObj(HotelMasterObj[p]);
        }
    }
...
function makeAjaxWithoutLoader(url, type, funname, data = null) {
    if (type == 'POST') {
        $.ajax({
            url: url,
            type: type,
...
$(document).ready(function () {
    ...
    var countryCode = '';
    countryCode += '<option data-countryCode="IN" value="91">India (+91)</option>' +
